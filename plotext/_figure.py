...
##############################################
    ...
    def box(self, dates, data, xside = None, yside = None, orientation = None, color = None, label = None):
        self.monitor.draw_box(dates, data, xside = xside, yside = yside, orientation = orientation, color = color, label = label) if self._no_plots else [[self._get_subplot(row, col).candlestick(dates, data, orientation = orientation, colors = colors, label = label) for col in self._Cols] for row in self._Rows]
